...
`test/nightfleet.adversarial.test.js` runs 54 cases: full happy-path games (7 hits to `claimWin`, post-game reveal) and adversarial must-revert cases (double join, re-commit mid-game, wrong fleet size, coordinate out of range, firing on the same cell twice, firing before boards are committed, a player reporting on their own board, premature `claimWin`, premature or dishonest `revealBoard`, and more).
